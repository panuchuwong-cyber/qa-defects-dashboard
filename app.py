...
if "14 Days" in page:
    ...
    # === SEARCH BAR — wrapped in a glass card so it has breathing room
    # from the Quick Actions row above. ===
    st.markdown(
        """
<style>
    .search-spacer { height: 14px; }
    .search-card {
        background: white;
    ...
    .search-card .stTextInput input:focus {
    ...
    try:
        ...
        if scores.empty:
            ...
            chart_html = (
                ...
                '<canvas id="top5worst"></canvas>'
                '</div>'
                '<div id="top5worst-err" style="color:#FF5252; font-size:11px; margin-top:8px; display:none;"></div>'
                '</div>'
                '<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script>'
                '<script>'
                '(function(){'
                '  function makeChart(){'
                '    var canvas = document.getElementById("top5worst");'
                '    var errBox = document.getElementById("top5worst-err");'
                '    if (!canvas) return;'
                ...
                '          plugins:{legend:{display:false}, tooltip:{backgroundColor:"#000", titleColor:"#FFD700", bodyColor:"#fff", borderColor:"#FFD700", borderWidth:1, callbacks:{label: function(ctx){return " Score: " + ctx.parsed.x.toFixed(1) + " / 100";}}}},'
                '          scales:{x:{min:0,max:100,grid:{color:"rgba(255,215,0,0.10)"},ticks:{color:"#FFD700",font:{weight:"600",size:11}}}, y:{grid:{display:false},ticks:{color:"#fff",font:{weight:"600",size:12}}}}'
                '        }'
                '      });'
                '      return true;'
                '    } catch(e) {'
                '      if (errBox) { errBox.style.display = "block"; errBox.textContent = "Chart error: " + e.message; }'
                '      return false;'
                '    }'
                '  }'
                '  if (!makeChart()) {'
                '    requestAnimationFrame(function(){ makeChart(); });'
                '  }'
